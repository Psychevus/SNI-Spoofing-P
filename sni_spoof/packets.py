from __future__ import annotations

import struct


class PacketTemplateError(ValueError):
    """Raised when a packet template cannot be built or parsed safely."""


def _require_exact_bytes(name: str, value: bytes, size: int) -> None:
    if not isinstance(value, bytes):
        raise PacketTemplateError(f"{name} must be bytes")
    if len(value) != size:
        raise PacketTemplateError(f"{name} must be exactly {size} bytes")


class ClientHelloMaker:
    tls_ch_template_str = ""
    tls_ch_template = bytes.fromhex(tls_ch_template_str)
    template_sni = b"mci.ir"
    static1 = tls_ch_template[:11]
    static2 = b"\x20"
    static3 = tls_ch_template[76:120]
    static4 = tls_ch_template[127 + len(template_sni):262 + len(template_sni)]
    static5 = b"\x00\x15"
    tls_change_cipher = b"\x14\x03\x03\x00\x01\x01"
    tls_app_data_header = b"\x17\x03\x03"

    @classmethod
    def get_client_hello_with(cls, rnd: bytes, sess_id: bytes, target_sni: bytes, key_share: bytes) -> bytes:
        _require_exact_bytes("rnd", rnd, 32)
        _require_exact_bytes("sess_id", sess_id, 32)
        _require_exact_bytes("key_share", key_share, 32)
        if not isinstance(target_sni, bytes):
            raise PacketTemplateError("target_sni must be bytes")
        if not target_sni:
            raise PacketTemplateError("target_sni must not be empty")
        if len(target_sni) > 219:
            raise PacketTemplateError("target_sni is too long for the padded TLS template")

        server_name_ext = (
            struct.pack("!H", len(target_sni) + 5)
            + struct.pack("!H", len(target_sni) + 3)
            + b"\x00"
            + struct.pack("!H", len(target_sni))
            + target_sni
        )
        padding_size = 219 - len(target_sni)
        padding_ext = struct.pack("!H", padding_size) + (b"\x00" * padding_size)
        return (
            cls.static1
            + rnd
            + cls.static2
            + sess_id
            + cls.static3
            + server_name_ext
            + cls.static4
            + key_share
            + cls.static5
            + padding_ext
        )

    @classmethod
    def parse_client_hello(cls, client_hello_bytes: bytes) -> tuple[bytes, bytes, str, bytes]:
        if len(client_hello_bytes) != 517:
            raise PacketTemplateError("client hello must be exactly 517 bytes")
        rnd = client_hello_bytes[11:43]
        sess_id = client_hello_bytes[44:76]
        sni_len = struct.unpack("!H", client_hello_bytes[125:127])[0]
        tls_sni_bytes = client_hello_bytes[127:127 + sni_len]
        try:
            tls_sni = tls_sni_bytes.decode("ascii")
        except UnicodeDecodeError as exc:
            raise PacketTemplateError("client hello SNI is not ASCII") from exc
        key_share_index = 262 + len(tls_sni_bytes)
        key_share = client_hello_bytes[key_share_index:key_share_index + 32]
        expected = cls.get_client_hello_with(rnd, sess_id, tls_sni_bytes, key_share)
        if expected != client_hello_bytes:
            raise PacketTemplateError("client hello does not match the supported template")
        return rnd, sess_id, tls_sni, key_share

    @classmethod
    def get_client_response_with(cls, app_data1: bytes) -> bytes:
        return cls.tls_change_cipher + cls.tls_app_data_header + struct.pack("!H", len(app_data1)) + app_data1

    @classmethod
    def parse_client_response(cls, client_response_bytes: bytes) -> bytes:
        if len(client_response_bytes) < 32:
            raise PacketTemplateError("client response is too short")
        app_data1 = client_response_bytes[11:]
        if cls.get_client_response_with(app_data1) != client_response_bytes:
            raise PacketTemplateError("client response does not match the supported template")
        return app_data1


class ServerHelloMaker:
    tls_sh_template_str = "160303007a0200007603035e39ed63ad58140fbd12af1c6a37c879299a39461b308d63cb1dae291c5b69702057d2a640c5ca53fed0f24491baaf96347f12db603fd1babe6bc3ad0b6fbde406130200002e002b0002030400330024001d0020d934ed49a1619be820856c4986e865c5b0e4eb188ebd30193271e8171152eb4e"
    tls_sh_template = bytes.fromhex(tls_sh_template_str)
    static1 = tls_sh_template[:11]
    static2 = b"\x20"
    static3 = tls_sh_template[76:95]
    tls_change_cipher = b"\x14\x03\x03\x00\x01\x01"
    tls_app_data_header = b"\x17\x03\x03"

    @classmethod
    def get_server_hello_with(cls, rnd: bytes, sess_id: bytes, key_share: bytes, app_data1: bytes) -> bytes:
        _require_exact_bytes("rnd", rnd, 32)
        _require_exact_bytes("sess_id", sess_id, 32)
        _require_exact_bytes("key_share", key_share, 32)
        return (
            cls.static1
            + rnd
            + cls.static2
            + sess_id
            + cls.static3
            + key_share
            + cls.tls_change_cipher
            + cls.tls_app_data_header
            + struct.pack("!H", len(app_data1))
            + app_data1
        )

    @classmethod
    def parse_server_hello(cls, server_hello_bytes: bytes) -> tuple[bytes, bytes, bytes, bytes]:
        if len(server_hello_bytes) < 159:
            raise PacketTemplateError("server hello is too short")
        rnd = server_hello_bytes[11:43]
        sess_id = server_hello_bytes[44:76]
        key_share = server_hello_bytes[95:127]
        app_data1 = server_hello_bytes[138:]
        if cls.get_server_hello_with(rnd, sess_id, key_share, app_data1) != server_hello_bytes:
            raise PacketTemplateError("server hello does not match the supported template")
        return rnd, sess_id, key_share, app_data1
